...
public class TestCaseGeneratorPanda {
	...
	public TestCaseGeneratorPanda(String name, int numberReachPolicies,  int seed) {
		this.name = name;
	...
	public NFV generateNFV( int numberReachPolicies, Random rand) {
		...
		int numberClient = numberReachPolicies;
		int numberServer = 1;
		...
		for(int i = 0; i < numberClient; i++) {
			...
			element.setSource("120.125.52.25");
			element.setDestination("120.125.52.25");
		...
		for(int i = 0; i < numberReachPolicies; i++) {
			createPolicy(PName.REACHABILITY_PROPERTY, nfv, graph, allClients.get(i).getName(), allServers.get(0).getName());
		}
