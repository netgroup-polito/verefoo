...
public class TestCaseGeneratorAtomicPredicatesVerigraph {
	NFV nfv;
	String name;
	...
	public TestCaseGeneratorAtomicPredicatesVerigraph(String name, int numberAllocationPlaces, int numberWebClients, int numberWebServers, 
	...
	public NFV generateNFV(int numberAllocationPlaces, int numberWebClients, int numberWebServers, int numberReachPolicies, int numberIsPolicies, 
		...
		for(int i = 0; i < numberAllocationPlaces; i++) {
			...
			Node ap = new Node();
			ap.setName(ip);
			ap.setFunctionalType(FunctionalTypes.FORWARDER);
			allAPs.add(ap);
		}
...
//		}
		...
		//create the policies: within Verigraph only reachability policies
		int numberRPWithPorts = (int) (numberReachPolicies * percReqWithPorts);
		for(int i = 0; i < numberReachPolicies + numberIsPolicies; i++) {
