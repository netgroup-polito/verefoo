...
public class PolitoCache extends NetworkObject{
    ...
    public void installCache (NetworkObject[] internalNodes){
           ...
           //Constraint2 isInCache(u_0), exist p_1, p_2, n_1, n_2 :
           //    (    recv(n_1, politoCache, p_1) && recv(n_2, politoCache, p_2))) &&
                           ...
                           ctx.mkImplies((BoolExpr)isInCache.apply(u_0),
                                   ctx.mkExists(new Expr[]{p_1,n_1},
                                           ctx.mkAnd(
                                                   ...
                                                   ctx.mkNot((BoolExpr)isInternalNode.apply(n_1)),
                                                   ctx.mkEq(nctx.pf.get("url").apply(p_1), u_0)),1,null,null,null,null)),1,null,null,null,null));
           //    //Always in cache
           ...
           //send(politoCache, n_0, p) && !isInternalNode(n_0) ->
           //    (exist n_1 :
           //        ( recv(n_1, politoCache, p) &&
           //         p.proto == HTTP_REQ &&  !isInCache(p.url))
           constraints.add(
           ...
           //Constraint4 send(politoCache, n_0, p) && isInternalNode(n_0) ->
           //    (exist p_1 :
           //        ( recv(n_0, politoCache, p_1) &&
           ...
           constraints.add(ctx.mkForall(new Expr[]{n_0,p_0},  ctx.mkImplies(
           			ctx.mkAnd((BoolExpr)nctx.recv.apply(n_0, politoCache, p_0),(BoolExpr)isInternalNode.apply(n_0)), 
           		 ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_REQUEST))
           		) ,1,null,null,null,null));
           
           constraints.add(ctx.mkForall(new Expr[]{n_0,p_0},  ctx.mkImplies(
       			ctx.mkAnd((BoolExpr)nctx.recv.apply(n_0, politoCache, p_0),ctx.mkNot((BoolExpr)isInternalNode.apply(n_0))), 
       	        ctx.mkAnd(ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_RESPONSE)), ctx.mkExists(new Expr[]{p_1},
       	        		  ctx.mkAnd((BoolExpr)nctx.send.apply( politoCache,n_0, p_1),
