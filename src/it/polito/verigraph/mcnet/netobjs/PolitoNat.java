...
public class PolitoNat extends NetworkObject {
	...
	public void natModel(DatatypeExpr natIp) {
		...
		Expr a = ctx.mkConst("a", nctx.node);
        Expr b = ctx.mkConst("b", nctx.node);
		...
		// p_0.<...>) "
		constraints.add(ctx.mkForall(new Expr[] { p_0, x }, ctx.mkImplies(
				...
				ctx.mkAnd(ctx.mkEq(nctx.pf.get("src").apply(p_0), natIp),
						ctx.mkExists(new Expr[] { y, p_1 }, ctx.mkAnd((BoolExpr) nctx.recv.apply(y, nat, p_1),
		...
		constraints.add(ctx.mkForall(new Expr[] { x, p_0 },
				ctx.mkImplies(
						...
						ctx.mkAnd(ctx.mkNot((BoolExpr) private_addr_func.apply(nctx.pf.get("src").apply(p_0))),
								ctx.mkExists(new Expr[] { y, p_1 }, ctx.mkAnd(
										...
										ctx.mkExists(new Expr[] { z, p_2 }, ctx.mkAnd(
