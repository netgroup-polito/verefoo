...
public class Checker {
	 ...
	 * NetworkObject dest){ assert(net.elements.contains(src));
	 * assert(net.elements.contains(dest)); solver.push (); addConstraints();
	 * 
	 * Expr p =
	 * ctx.mkConst("check_isolation_p_"+src.getZ3Node()+"_"+dest.getZ3Node(),
	 * nctx.packet); Expr n_0
	 * =ctx.mkConst("check_isolation_n_0_"+src.getZ3Node()+"_"+dest.getZ3Node(),
	 * nctx.node); Expr n_1
	 * =ctx.mkConst("check_isolation_n_1_"+src.getZ3Node()+"_"+dest.getZ3Node(),
	 * nctx.node); Expr n_2
	 * =ctx.mkConst("check_isolation_n_1_"+src.getZ3Node()+"_"+dest.getZ3Node(),
	 * nctx.node);
	 * 
	 * IntExpr t_0 =
	 * ctx.mkIntConst("check_isolation_t0_"+src.getZ3Node()+"_"+dest.getZ3Node()
	 * ); IntExpr t_1 =
	 * ctx.mkIntConst("check_isolation_t1_"+src.getZ3Node()+"_"+dest.getZ3Node()
	 * ); IntExpr t_2 =
	 * ctx.mkIntConst("check_isolation_t2_"+src.getZ3Node()+"_"+dest.getZ3Node()
	 * );
	 * 
	 * // Constraint1recv(n_0,destNode,p,t_0)
	 * this.solver.add((BoolExpr)nctx.recv.apply(n_0, dest.getZ3Node(), p,
	 * t_0));
	 * 
	 * // Constraint2send(srcNode,n_1,p1,t_1)
	 ...
	 * this.solver.add((BoolExpr)nctx.nodeHasAddr.apply(src.getZ3Node(),
	 * nctx.pf.get("src").apply(p)));
	 * 
	 * // Constraint4p.origin == srcNode
	 * this.solver.add(ctx.mkEq(nctx.pf.get("origin").apply(p),
	 * src.getZ3Node()));
	 * 
	 * 
	 * Expr p_2 =
	 * ctx.mkConst("check_isolation_p_flow_"+src.getZ3Node()+"_"+dest.getZ3Node(
	 * ), nctx.packet);
	 * 
	 * // Constraint5there does not exist p_2, n_2, t_2 : //
	 * send(destNode,n_2,p_2,t_2) && // p_2.srcAddr == p. destAddr && //
	 * p_2.srcPort == p.destPort && // p_2.destPort == p.srcPort && // p_2.destt
	 * == p.src && // t_2 < t_0 this.solver.add(ctx.mkNot(ctx.mkExists(new
	 * Expr[]{p_2,n_2,t_2}, ctx.mkAnd(
	 ...
	 * ctx.mkAnd( (BoolExpr)nctx.send.apply(dest.getZ3Node(), n_2, p_2, t_2),
	 * ctx.mkEq(nctx.pf.get("src").apply(p_2), nctx.pf.get("dest").apply(p)),
	 * ctx.mkEq(nctx.src_port.apply(p_2), nctx.dest_port.apply(p)),
	 * ctx.mkEq(nctx.dest_port.apply(p_2), nctx.src_port.apply(p)),
	 * ctx.mkEq(nctx.pf.get("dest").apply(p_2), nctx.pf.get("src").apply(p)),
	 * ctx.mkLt(t_2,t_0)),1,null,null,null,null))));
	 * 
	 * result = this.solver.check(); model = null; assertions =
	 * this.solver.getAssertions(); if (result == Status.SATISFIABLE){ model =
	 * this.solver.getModel(); } this.solver.pop(); return new
	 * IsolationResult(ctx,result, p, n_0, t_1, t_0, nctx, assertions, model); }
	 * 
	 * 
	 * 
	 * public IsolationResult CheckNodeTraversalProperty (NetworkObject src,
	 * NetworkObject dest, NetworkObject node){
	 * assert(net.elements.contains(src)); assert(net.elements.contains(dest));
	 * solver.push (); addConstraints();
	 * 
	 * Expr p =
	 * ctx.mkConst("check_isolation_p_"+src.getZ3Node()+"_"+dest.getZ3Node(),
	 * nctx.packet);
	 ...
	 * 
	 * 
	 * //Constraint5there does not exist n_2, t_2 : recv(n_2,node,p,t_2) && t_2
	 * < t_0 this.solver.add(ctx.mkNot(ctx.mkExists(new Expr[]{n_2,t_2},
	 * ctx.mkAnd( (BoolExpr)nctx.recv.apply(n_2, node.getZ3Node(), p, t_2),
	 * ctx.mkLt(t_2,t_0)),1,null,null,null,null)));
	 * 
	 * //Constraint 6there does not exist n_2, t_2 : send(node,n_2,p,t_2) && t_2
	 * < t_0 this.solver.add(ctx.mkNot(ctx.mkExists(new Expr[]{n_2,t_2},
	 * ctx.mkAnd( (BoolExpr)nctx.send.apply(node.getZ3Node(), n_2, p, t_2),
	 * ctx.mkLt(t_2,t_0)),1,null,null,null,null)));
	 * 
	 * 
	 * result = this.solver.check(); model = null; assertions =
	 ...
	 * }
	 * 
	 * public IsolationResult CheckLinkTraversalProperty (NetworkObject src,
	 * NetworkObject dest, NetworkObject le0, NetworkObject le1){
	 * assert(net.elements.contains(src)); assert(net.elements.contains(dest));
	 * solver.push (); addConstraints();
	 * 
	 * Expr p =
	 ...
	 * src.getZ3Node()));
	 * 
	 * //Constraint5∃ t_1, t_2 : // send(linkNode0,linkNode1,p,t_1) && //
	 * recv(linkNode0,linkNode1,p,t_2) && // t_1 < t_0 && // t_2 < t_0
	 * this.solver.add(ctx.mkExists(new Expr[]{t_1,t_2}, ctx.mkAnd(
	 * (BoolExpr)nctx.send.apply(le0.getZ3Node(), le1.getZ3Node(), p, t_1),
	 * (BoolExpr)nctx.recv.apply(le0.getZ3Node(), le1.getZ3Node(), p, t_2),
	 * ctx.mkLt(t_1,t_0), ctx.mkLt(t_2,t_0)),1,null,null,null,null));
	 * 
	 * 
	 * result = this.solver.check(); model = null; assertions =
	 * this.solver.getAssertions(); if (result == Status.SATISFIABLE){ model =
	 ...
	 * }
	 * 
	 * public Result CheckDataIsolationPropertyCore (NetworkObject src,
	 * NetworkObject dest){ assert(net.elements.contains(src));
	 * assert(net.elements.contains(dest)); List<BoolExpr> constr = new
	 * ArrayList(this.getConstraints());
	 * 
	 * Expr p =
	 * ctx.mkConst("check_isolation_p_"+src.getZ3Node()+"_"+dest.getZ3Node(),
	 * nctx.packet);
	 ...
	 * 
	 * IntExpr t =
	 * ctx.mkIntConst("check_isolation_t_"+src.getZ3Node()+"_"+dest.getZ3Node())
	 * ;
	 * 
	 ...
	 * this.solver.push();
	 * 
	 * // Constraint3for each constraint( n -> constraint) ArrayList<BoolExpr>
	 * names =new ArrayList<BoolExpr>(); for(BoolExpr con : constr){ BoolExpr n
	 * = ctx.mkBoolConst(""+con); names.add(n); this.solver.add(ctx.mkImplies(n,
	 * con)); }
	 * 
	 * BoolExpr[] nam = new BoolExpr[names.size()]; result =
	 * this.solver.check(names.toArray(nam)); Result ret =null;
	 * 
	 * if (result == Status.SATISFIABLE){ System.out.println("SAT"); ret = new
	 * Result(ctx,this.solver.getModel()); }else if(result ==
	 * Status.UNSATISFIABLE){ System.out.println("unsat"); ret = new
	 * Result(ctx,this.solver.getUnsatCore()); } this.solver.pop(); return ret;
	 * }
	 * 
	 * public DataIsolationResult CheckDataIsolationProperty(NetworkObject src,
	 * NetworkObject dest){ assert(net.elements.contains(src));
	 * assert(net.elements.contains(dest)); solver.push (); addConstraints();
	 * 
	 * Expr p =
	 ...
	 * this.solver.getAssertions(); if (result == Status.SATISFIABLE){ model =
	 * this.solver.getModel(); } this.solver.pop(); return new
	 * DataIsolationResult(ctx,result, p, n_0, t, nctx, assertions, model); }
	 */
